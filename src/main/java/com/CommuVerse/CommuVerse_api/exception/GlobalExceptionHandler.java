...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<UserErrorResponse> handleResourceNotFoundException(ResourceNotFoundException ex, WebRequest request) {
        UserErrorResponse err = new UserErrorResponse(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(err, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(BadRequestException.class)
    public ResponseEntity<UserErrorResponse> handleBadRequestException(BadRequestException ex, WebRequest request) {
        UserErrorResponse err = new UserErrorResponse(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
        return new ResponseEntity<>(err, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<UserErrorResponse> handleAllExceptions(Exception ex, WebRequest request) {
        UserErrorResponse err = new UserErrorResponse(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
